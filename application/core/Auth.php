 ...
 class Auth 
 {
	...
	public static function checkAuthentication()
    {
        ...
        if (!Session::userIsLoggedIn()) {
            ...
            // successful login
            header('location: ' . Config::get('URL') . 'auth?redirect=' . urlencode($_SERVER['REQUEST_URI']));
	...
	public static function setAuthValue() 
	{
		// initialize the session (if not initialized yet)
        Session::init();
		...
		// if user is NOT logged in...
        // (if user IS logged in the application will and just go on)
		
		if(!Session::userIsLoggedIn()) {
			// do notting and procedd to else just show login button
		}else {
			
			// check user img details and profile link 
			// check if user is admin first
			if(Session::get("user_account_type") != 7) {
				// if user is admin redirect straight to admin section
				header('location: ' . Config::get('URL') . 'admin');
