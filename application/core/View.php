...
class View
{
	...
	/**
     * this is to set a different template , same as render()
     */
	public function renderWithDifferentTemplates($filename, $data = null, $templates)
    {
        if ($data) {
            foreach ($data as $key => $value) {
                $this->{$key} = $value;
        ...
        require Config::get('PATH_VIEW') . '_templates/'. $templates .'/header.php';
        require Config::get('PATH_VIEW') . $filename . '.php';
        require Config::get('PATH_VIEW') . '_templates/'. $templates .'/footer.php';
    }
